...
> インストールされる APK と現在の画面は繁体字中国語です。Rokid は一般消費者向け YodaOS の Android 動作をすべて公開仕様として保証しているわけではありません。本格的に使用する前に、[実機検証チェックリスト](docs/DEVICE_CHECK.ja.md)に従って、ファイルパス、ストレージ権限、タッチパッドイベントを確認してください。

## 機能

- 眼鏡向け 480×640、黒背景・高コントラストの画面：軽いスワイプは 1 項目、高速フリングは 2～8 項目移動し、タップで実行、ダブルタップで戻ります。プレビュー中のスワイプで前後の項目へ直接切り替えられます。
- 写真／動画／日付別の閲覧に対応。縮小サムネイルは上限 8 MB の LRU キャッシュを使用し、12MP の原画像全体をメモリへ読み込みません。
- スマートフォンまたはパソコンのブラウザから、ファイル一覧、プレビュー、Range ストリーミング、明示的なダウンロードを利用可能。原本を事前同期しません。
- 名前変更、ゴミ箱への移動、復元、複数ファイルのアップロードに対応。ゴミ箱では項目ごとの完全削除と一括削除ができ、2 回の確認を行い、保護済み項目は残します。
- 重複ファイル検索は、サイズ、先頭 64 KiB の指紋、完全な SHA-256 の順で比較し、大きな動画の不要な読み取りを減らします。
- アプリ内にはパスなどを除去した直近 200 件の診断イベントだけを保持し、利用者が明示的に選んだ場合のみ書き出します。外部送信は行いません。
...
- スマートフォンで管理している間は眼鏡画面を常時点灯し、共有停止またはタイムアウト時に通常のスリープへ戻ります。眼鏡に表示された URL とペアリングコードをスマートフォンへ直接入力します。
...
2. 眼鏡で `同 Wi‑Fi 手機管理`（同一 Wi‑Fi のスマートフォン管理）を開きます。
3. スマートフォンのブラウザへ、眼鏡に表示された `http://私有IP:8765` を手入力し、その都度生成されるペアリングコードを入力します。
4. 終了後、直ちに共有を停止します。
