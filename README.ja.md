...
- Wi‑Fi 共有 URL の QR コードは眼鏡内で完全にオフライン生成されます。スマートフォンで管理している間は眼鏡画面を常時点灯し、共有停止またはタイムアウト時に通常のスリープへ戻ります。
...
3. スマートフォンのカメラで眼鏡に表示されたオフライン QR コードを読み取るか、`http://私有IP:8765` を手入力し、その都度生成されるペアリングコードを入力します。
4. 終了後、直ちに共有を停止します。
