...
> 「Rokid 眼鏡ファイル管理アプリ」は説明文での日本語名です。インストールされる APK と現在の画面は繁体字中国語のままで、ランチャーに表示される固定名は `Rokid眼鏡檔案管理APP` です。Rokid は一般消費者向け YodaOS の Android 動作をすべて公開仕様として保証しているわけではありません。本格的に使用する前に、[実機検証チェックリスト](docs/DEVICE_CHECK.ja.md)に従って、ファイルパス、ストレージ権限、タッチパッドイベントを確認してください。
...
```
SHA-256：`f73678a7035bbee60b5b9ee059ed08f9f1941d08a2c308879352fd851ee72a1c`。署名証明書フィンガープリント：`4474b01e8e0fd74d484775e223344ca7703c779719d6a634621cce0d782fd9d8`。
...
同梱の磁気充電ケーブルは、データ通信に対応していない場合があります。インストール後は固定表示名 `Rokid眼鏡檔案管理APP` を探してください。通常は眼鏡のアプリ一覧の末尾に表示されますが、この動作は実機のファームウェアによって異なる可能性があります。
