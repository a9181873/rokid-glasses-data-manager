...
> 專案只提供一個可安裝版本，App 名稱固定為「眼鏡檔案站」。Rokid 並未把消費版 YodaOS 的所有 Android 行為列為公開契約；正式使用前必須依 [真機驗收清單](docs/DEVICE_CHECK.md) 測試檔案路徑、儲存授權與觸控板事件。
...
本專案只有一個版本，名稱固定為「眼鏡檔案站」。首次啟動需明確允許 Android 的「所有檔案存取」；這讓手機瀏覽器能在不逐檔回到眼鏡確認的情況下重新命名、移動及復原媒體。程式內仍硬性限制在 `DCIM/Camera`、`DCIM/album`、`Pictures`、`Movies` 等媒體白名單，並拒絕符號連結與 canonical path 逃逸；未授權時只提供 MediaStore 唯讀備援。
