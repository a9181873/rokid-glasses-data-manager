...
- 眼鏡端 480×640 黑底高對比介面：輕滑逐項、快滑依速度跳 2–8 項、點按執行、雙擊返回；預覽中滑動可直接換上一張／下一張。
- 依相片／影片／日期瀏覽，採樣縮圖並使用 8 MB 有界 LRU 快取，不把 12MP 原圖整張載入記憶體。
- 手機或電腦瀏覽器直接列檔、預覽、Range 串流與明確下載，不預先同步原檔。
- 重新命名、移至垃圾桶、還原及多檔上傳；垃圾桶支援逐檔永久刪除與清空，兩次確認並保留受保護項目。
- 重複檔掃描依序比對檔案大小、前 64 KiB 指紋及完整 SHA-256，降低大型影片的無效讀取。
- App 內只保留最近 200 筆、去除路徑與敏感值的診斷事件；使用者主動選擇時才匯出，不傳送到外部。
- USB 模式只監聽眼鏡本機 `127.0.0.1:8765`，搭配 ADB port forwarding。
- Wi‑Fi 模式只監聽當下的私有 IPv4；每次啟動重建配對碼與 256-bit 工作階段權杖，10 分鐘無操作自動停止。
- 手機管理期間保持眼鏡螢幕常亮，停止或閒置逾時即恢復一般休眠；手機直接輸入眼鏡顯示的網址與 PIN。
...
2. 在眼鏡開啟「同 Wi‑Fi 手機管理」。
3. 在手機瀏覽器手動輸入眼鏡顯示的 `http://私有IP:8765`，再輸入當次配對碼。
4. 完成後立即停止分享。
