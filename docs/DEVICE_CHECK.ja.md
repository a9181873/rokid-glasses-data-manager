...
前方／後方への軽いスワイプ、高速フリング、タップ、ダブルタップ、長押し、戻る、DPAD を 1 つずつ試してください。軽いスワイプは 1 項目、高速フリングは最大 8 項目だけ移動し、方向が反転せず、細切れの Generic Motion で連続ジャンプしないことを確認します。プレビューでは 1 回のスワイプで 1 項目だけ切り替わり、位置カウンターも更新される必要があります。製品版ファームウェアが shell からの `/dev/input` 読み取りを拒否する場合は、`adb shell dumpsys input` でデバイス／キー設定を保存し、各ジェスチャーに対する画面の反応を記録します。アプリは標準 DPAD、Generic Motion、Touch MotionEvent に対応しています。OEM 独自イベントが異なる場合は、実機の証拠に基づくマッピングが必要です。
...
テスト用ファイルだけを使い、名前変更、ゴミ箱への移動、復元、項目ごとの完全削除、ゴミ箱の一括削除、アップロードの中断、ファイル名の重複、空き容量不足、2GB を超える動画を検証します。完全削除で 2 回の確認が必要なこと、保護済み項目が残ること、処理に失敗しても公開済みの不完全なファイルが残らないことを確認してください。
